...
interface FileUploaderProps {
	fileUrl: string | null;
	fileType: string | null; // To track whether it's an image or video
	handleFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
	handleDrop: (e: React.DragEvent<HTMLDivElement>) => void;
	handleDragOver: (e: React.DragEvent<HTMLDivElement>) => void;
	handleDragLeave: () => void;
	handleRemoveFile: () => void; // For removing the file
	dragging: boolean;
	loading: boolean;
	uploaded: boolean;
	handleUpdateFile: (e: React.ChangeEvent<HTMLInputElement>) => void; // For updating the file
}
...
const FileUploader: React.FC<FileUploaderProps> = ({
	...
	return (
		...
		>
			...
			{!uploaded ? (
				<div className="flex flex-col items-center gap-2">
					{fileUrl && fileType?.startsWith('image/') ? (
						<img
							src={"https://landing-page-imgg.s3.eu-north-1.amazonaws.com/banner-img/939850629aae90dd234d"} // Use empty string if fileUrl is undefined
							alt="Preview"
						...
						{!loading ? (
							<label className="bg-teal-600 text-white py-2 px-7 rounded cursor-pointer inline-block">
								...
								<input
									type="file"
									className="hidden"
									accept=".png, .jpeg, .jpg, .mp4, .avi, .mkv, .mov"
			...
			<p className="text-gray-900 mt-4">Supported formats: PNG, JPEG, JPG, MP4, AVI, MKV, MOV up to 50 MB</p>
		</div>
	);
};
